...
void H2ERI_matvec_intmd_mult_AOTtest(H2ERI_p h2eri, const double *x)
{
    int n_node       = h2eri->n_node;
    int n_thread     = h2eri->n_thread;
    int *r_adm_pairs = h2eri->r_adm_pairs;
...
// in the matvec test, only the dense part of the matrix participate in the computation
...
// in test1 all the B elements are changed to 0s
void H2ERI_matvectest1(H2ERI_p h2eri, const double *x, double *y)
{
    ...
    if (h2eri->BD_JIT == 0)
    {
        H2ERI_matvec_intmd_mult_AOTtest(h2eri, x);
    } else {
        H2ERI_matvec_intmd_mult_JIT(h2eri, x);
    }
    et = get_wtime_sec();
...
// H2 representation for an ERI tensor multiplies a column vector
void H2ERI_matvectestdiag(H2ERI_p h2eri, const double *x, double *y)
{
    int krnl_mat_size = h2eri->krnl_mat_size;
    int n_thread      = h2eri->n_thread;
    H2E_thread_buf_p *thread_buf = h2eri->thread_buffs;
    ...
    if (h2eri->BD_JIT == 0)
    {
        H2ERI_matvec_dense_mult_AOTdiag(h2eri, x);
    } else {
        H2ERI_matvec_dense_mult_JIT(h2eri, x);
    }
    et = get_wtime_sec();
