...
int main(int argc, char **argv)
{
    ...
    double *testdmat;
    double* testjmat;
    testdmat=(double*) malloc(sizeof(double) * numbfp*numbfp);
    testjmat=(double*) malloc(sizeof(double) * numbfp*numbfp);
    memset(testdmat, 0, sizeof(double) * numbfp*numbfp);
    memset(testjmat, 0, sizeof(double) * numbfp*numbfp);
    for(int i=0;i<numbfp;i++)
    {
        testdmat[i*numbfp+i]=1;
    }
    #pragma omp parallel for
    for(int i=0;i<numbfp;i++)
    {
        double* testdptr=testdmat+i*numbfp;
        double* testjptr=testjmat+i*numbfp;
        H2ERI_matvec(h2eri,testdptr, testjptr);
    ...
    double norm;
    norm=Calc2norm(testdmat,numbfp);
    printf("d should be numbfp.%f\n",norm);
    norm=Calc2norm(testjmat,numbfp);
    printf("j should be %f\n",norm);
    /*
    COOmat_p cooh2d1;
    COOmat_init(&cooh2d1,h2eri->num_bf*h2eri->num_bf,h2eri->num_bf*h2eri->num_bf);
    H2ERI_build_COO_Diamattest(h2eri,cooh2d1,1,0);
    CSRmat_p csr;
    CSRmat_init(&csr,h2eri->num_bf,h2eri->num_bf);
    Double_COO_to_CSR( h2eri->num_bf,  ndc, cooh2d1,csr);
    CSRmat_p csr;
    CSRmat_init(&csr,h2eri->num_bf,h2eri->num_bf);
    Double_COO_to_CSR( h2eri->num_bf,  ndc, cooh2d1,csr);
