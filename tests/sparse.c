...
int main(int argc, char **argv)
{
    ...
    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    */
    // Test the largest 10*nbf elements in D and DC and D+DC
    double *xy;
    xy = (double *) malloc(sizeof(double) * nbf * nbf);
    memset(xy, 0, sizeof(double) * nbf * nbf);
    for (int i = 0; i < nbf; i++)
    {
        for (int j = 0; j < nbf; j++)
        {
            xy[i * nbf + j] = TinyDFT->D_mat[i * nbf + j] + TinyDFT->DC_mat[i * nbf + j];
        }
    }
    double nmx = 0;
    double nmy = 0;
    ...
    for (int i = 0; i < nbf; i++)
    {
        nmx += TinyDFT->D_mat[i * nbf + i] * TinyDFT->D_mat[i * nbf + i];
        nmy += TinyDFT->DC_mat[i * nbf + i] * TinyDFT->DC_mat[i * nbf + i];
        nmxy += xy[i * nbf + i] * xy[i * nbf + i];
    }
    printf("The norm of D is %f while the norm of the first 10*nbf elements is %f and the rate is %f\n",Calc2norm(TinyDFT->D_mat,nbf*nbf),Calc2norm(TinyDFT->D_mat,10*nbf),Calc2norm(TinyDFT->D_mat,10*nbf)/Calc2norm(TinyDFT->D_mat,nbf*nbf));
    printf("The norm of DC is %f while the norm of the first 10*nbf elements is %f and the rate is %f\n",Calc2norm(TinyDFT->DC_mat,nbf*nbf),Calc2norm(TinyDFT->DC_mat,10*nbf),Calc2norm(TinyDFT->DC_mat,10*nbf)/Calc2norm(TinyDFT->DC_mat,nbf*nbf));
    printf("The norm of D+DC is %f while the norm of the first 10*nbf elements is %f and the rate is %f\n",Calc2norm(xy,nbf*nbf),Calc2norm(xy,10*nbf),Calc2norm(xy,10*nbf)/Calc2norm(xy,nbf*nbf));
    printf("The number of basis functions is %d\n",nbf);
    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    sortArray(TinyDFT->D_mat,nbf*nbf);
    sortArray(TinyDFT->DC_mat,nbf*nbf);
    sortArray(xy,nbf*nbf);
    printf("The norm of D is %f while the norm of the first 10*nbf elements is %f and the rate is %f, and diagonal rate is %f\n",Calc2norm(TinyDFT->D_mat,nbf*nbf),Calc2norm(TinyDFT->D_mat,10*nbf),Calc2norm(TinyDFT->D_mat,10*nbf)/Calc2norm(TinyDFT->D_mat,nbf*nbf),nmx/Calc2norm(TinyDFT->D_mat,nbf*nbf));
    printf("The norm of DC is %f while the norm of the first 10*nbf elements is %f and the rate is %f, and diagonal rate is %f\n",Calc2norm(TinyDFT->DC_mat,nbf*nbf),Calc2norm(TinyDFT->DC_mat,10*nbf),Calc2norm(TinyDFT->DC_mat,10*nbf)/Calc2norm(TinyDFT->DC_mat,nbf*nbf),nmy/Calc2norm(TinyDFT->DC_mat,nbf*nbf));
    printf("The norm of D+DC is %f while the norm of the first 10*nbf elements is %f and the rate is %f, and diagonal rate is %f\n",Calc2norm(xy,nbf*nbf),Calc2norm(xy,10*nbf),Calc2norm(xy,10*nbf)/Calc2norm(xy,nbf*nbf),nmxy/Calc2norm(xy,nbf*nbf));
    printf("The number of basis functions is %d\n",nbf);

    sortArray(TinyDFT->Cocc_mat,nbf*TinyDFT->n_occ);
    sortArray(TinyDFT->Cvir_mat,nbf*TinyDFT->n_vir);
    printf("The norm of OCC is %f while the norm of the first 10*nbf elements is %f and the rate is %f\n",Calc2norm(TinyDFT->Cocc_mat,nbf*TinyDFT->n_occ),Calc2norm(TinyDFT->Cocc_mat,10*nbf),Calc2norm(TinyDFT->Cocc_mat,10*nbf)/Calc2norm(TinyDFT->Cocc_mat,nbf*TinyDFT->n_occ));
    printf("The norm of VIR is %f while the norm of the first 10*nbf elements is %f and the rate is %f\n",Calc2norm(TinyDFT->Cvir_mat,nbf*TinyDFT->n_vir),Calc2norm(TinyDFT->Cvir_mat,10*nbf),Calc2norm(TinyDFT->Cvir_mat,10*nbf)/Calc2norm(TinyDFT->Cvir_mat,nbf*TinyDFT->n_vir)); 
    printf("The number of basis functions is %d\n",nbf);
