...
void TestCOO(COOmat_p coomat)
{
    ...
    for(size_t i=0;i<coomat->nnz/20;i++)
    {
...
//    size_t nnz=cooh2d->nnz;
    ...
    printf("Now start to build exchange matrix\n");
    COOmat_p cooexc;
    COOmat_init(&cooexc,h2eri->num_bf*h2eri->num_bf,h2eri->num_bf*h2eri->num_bf);
    H2ERI_build_COO_transpose(cooh2d1,cooexc,nbf);
    printf("The first value in cooh2d1 is %d,%d, %f\n",cooh2d1->coorow[0],cooh2d1->coocol[0],cooh2d1->cooval[0]);
    printf("The first value in cooexc is %d,%d, %f\n",cooexc->coorow[0],cooexc->coocol[0],cooexc->cooval[0]);
    printf("finish building exchange matrix\n");
    TestCOO(cooexc);
...
//    printf("The energy is %f\n",energy);
    ...
    CSRmat_p excgdle;
    CSRmat_init(&excgdle,h2eri->num_bf*h2eri->num_bf,h2eri->num_bf*h2eri->num_bf);
    st1 = get_wtime_sec();
    Xindextransform1(h2eri->num_bf,csrexc,csrden,excgdle);
    et1 = get_wtime_sec();
    printf("The X Index transformation time is %.3lf (s)\n",et1-st1);
    TestCSR(gdle);
    printf("Xindex transformation finished\n");
    CSRmat_p excgdls;
    CSRmat_init(&excgdls,h2eri->num_bf*h2eri->num_bf,h2eri->num_bf*h2eri->num_bf);
    
    st1 = get_wtime_sec();
    Yindextransform1(h2eri->num_bf,excgdle,csrdc,excgdls);
    et1 = get_wtime_sec();
    
    TestCSR(excgdls);
    printf("The Y Index transformation time is %.3lf (s)\n",et1-st1);
    double energy;
    energy = Calc_S1energy(excgdls,colgdls);
    printf("The energy is %f\n",energy);
    et1 = get_wtime_sec();
