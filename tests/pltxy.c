...
int main(int argc, char **argv)
{
    ...
    */
    ...
    FILE *file = fopen("outputdist.txt", "w");
    if (file == NULL) {
    ...
    for (int i = 0; i < nbf; i++) {
        for (int j = 0; j < nbf; j++) {
            fprintf(file, "%.16g ", distance[i*nbf+j]);
        }
        fprintf(file, "\n");
    }
    fclose(file);

    FILE *file1 = fopen("outputCOCC.txt", "w");
    if (file1 == NULL) {
        printf("Error opening file!\n");
        return 1;
    }
    ...
    for (int i = 0; i < nbf; i++) {
        for (int j = 0; j < TinyDFT->n_occ; j++) {
            fprintf(file1, "%.16g ", TinyDFT->Cocc_mat[i*TinyDFT->n_occ+j]);
        }
        fprintf(file1, "\n");
    }
    ...
    FILE *file4 = fopen("outputS.txt", "w");
    if (file4 == NULL) {
        printf("Error opening file!\n");
        return 1;
    }
    ...
    for (int i = 0; i < nbf; i++) {
        for (int j = 0; j < nbf; j++) {
            fprintf(file4, "%.16g ", TinyDFT->S_mat[i*nbf+j]);
        }
        fprintf(file4, "\n");
    }
