...
int main(int argc, char **argv)
{
    ...
    for(int quad = 0;quad<n;quad++)
    {
        ...
        size_t nfq = H2ERI_build_S5_draft(h2eri,Urbasis,Ucbasis,csrd5,csrdc5,npairs,pair1st,pair2nd,nodepairs,nodeadmpairs,nodeadmpairidx,S51cbasis,Upinv,thresprod);
        et1 = get_wtime_sec();
        ...
        printf("The S51 energy in quad %d is %.16g\n",quad,s51energy);


        
        
        s1s5 = calc_S1S51(gdls,h2eri, Urbasis,S51cbasis, nodepairs, nodepairidx);
        printf("The S1S51 energy in quad %d is %.16g\n",quad,s1s5);
        printf("The total energy in quad %d is %.16g\n",quad,energy+2*s1s5+s51energy);
